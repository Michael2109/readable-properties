    ...
    it("Should parse a property group with nested property groups"){
      val code =
        """groupName1:
          ...
          |  z:3
        """.stripMargin.replace("\r", "")
      TestUtil.parse(code, StatementParser.propertySourceParser) shouldBe PropertyContainer(ArrayBuffer(PropertyGroup(Identifier("groupName1"),ArrayBuffer(PropertyElement(Identifier("x"),IntConst(1)), PropertyGroup(Identifier("groupName2"),ArrayBuffer(PropertyElement(Identifier("a"),IntConst(1)), PropertyElement(Identifier("b"),IntConst(2)), PropertyElement(Identifier("c"),IntConst(3)))), PropertyElement(Identifier("y"),IntConst(2)), PropertyElement(Identifier("z"),IntConst(3))))))
    }
